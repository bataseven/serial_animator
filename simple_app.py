...
class AddComport(QMainWindow):
    ...
    def __init__(self, parent, menu):
        ...
        menuComport = menu.addMenu("Com Ports")

        info_list = QSerialPortInfo()
        serial_list = info_list.availablePorts()
        serial_ports = [port.portName() for port in serial_list]
        if len(serial_ports) > 0:
            portCount = len(serial_ports)
            count = 0
            while count < portCount:
                button_action = QAction(serial_ports[count], self)
                txt = serial_ports[count]
                portinfo = QSerialPortInfo(txt)
                buttoninfotxt = "No device information"
                if portinfo.hasProductIdentifier():
                    buttoninfotxt = (" Device Info: " + str(portinfo.vendorIdentifier()))
                if portinfo.hasVendorIdentifier():
                    buttoninfotxt = buttoninfotxt + (" Device ID: " + str(portinfo.productIdentifier()))
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.serial = QtSerialPort.QSerialPort(portname, baudRate=9600, readyRead=self.receive)
